...
int main() {
	...
	while(true) {
		...
		pts = poly;
		for(int i = 0; i < n - 1; i++) {
			point prePoint = i == 0? pts[n - 2] : pts[i - 1];
			double toBeCut = min(d, pts[i].dist(prePoint) - d);
			vec v = toVec(pts[i], pts[i + 1]).normalize();
			vec perpV(-v.y, v.x);
			point a = pts[i].translate(perpV.scale(toBeCut));
			point b = pts[i + 1].translate(perpV.scale(toBeCut));
