...
struct linesegment {
	...
	bool intersection(linesegment ls, point& p) {
		line l1(a, b);
		line l2(ls.a, ls.b);
		if(l1.parallel(l2)) {
			if(l1.same(l2))
		...
		return p.between(a, b) && p.between(ls.a, ls.b);
	}
};
...
int main() {
	...
	while(TC--) {
		...
		cin >> thr >> x;
		line cut(point(x, 0), point(x, 1));
		vector<point> pts;
		double len = 0;
		bool valid = true;
		for(int i = 0; i < N; i++) {
			linesegment ls(arr[i], arr[i+1]);
			point x;
			if(cut.intersection(line(ls.a, ls.b), x)) {
				if(x.onsegment(ls.a, ls.b)) {
					if(x == ls.a || x == ls.b) {
						valid = false;
						break;
					}
		...
		for(int i = 1; i < pts.size(); i+= 2) {
			len += abs(pts[i].y - pts[i-1].y);
		}
		cout << (thr - len < EPS && valid? "YES" : "NO") << '\n';
	}
	return 0;
}
