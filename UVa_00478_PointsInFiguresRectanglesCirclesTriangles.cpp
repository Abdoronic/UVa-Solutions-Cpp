...
vector< pair< char, vector<point> > > shapes;

bool inShape(int i, point p) {
	char type = shapes[i].first;
	vector<point> poly = shapes[i].second;
	if(type == 'c')
		return p.dist(poly[0]) - poly[1].x < -EPS;
	return inPolygon(poly, p);
}
...
int main() {
	...
	for(int i = 0; true; i++) {
		...
		vector<point> poly;
		if(type == 'r') {
			point topLeft(v[0], v[1]);
			point bottomRight(v[2], v[3]);
			...
			for(int i = 0; i < v.size(); i+= 2)
				poly.pb(point(v[i], v[i + 1]));
			poly.pb(poly.front());
		} else {
			poly.pb(point(v[0], v[1]));
			poly.pb(point(v[2], 0));
		}
		shapes.pb({type, poly});
	}
