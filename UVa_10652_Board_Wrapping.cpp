...
void getCorners(double x, double y, double w, double h, double angle, vector<point>& poly) {
	vec dy (cos(angle), sin(angle));
	vec dx (-dy.y, dy.x);
	dx = dx.scale(w / 2);
	dy = dy.scale(h / 2);
	vec negdx = dx.reverse();
	vec negdy = dy.reverse();
	point mid(x, y), topLeft, topRight, bottomLeft, bottomRight;
	topLeft = mid.translate(negdx).translate(dy);
	topRight = mid.translate(dx).translate(dy);
	bottomLeft = mid.translate(negdx).translate(negdy);
	bottomRight = mid.translate(dx).translate(negdy);
	poly.pb(topLeft); poly.pb(topRight);
	poly.pb(bottomLeft); poly.pb(bottomRight);
}
...
int main() {
	...
	while(t--) {
		...
		while (n--) {
			...
			getCorners(x, y, w, h, angle, poly);
			area += w * h;
		}
		convexHull(poly);
		double totalArea = polygonArea(poly);
		double perc = area * 100 / totalArea;
		printf("%.1f %%\n", perc);
	}
	return 0;
}
