...
int main() {
	...
	for(int t = 1; true; t++) {
		...
		vector<point> poly;
		vi ans, curr;
		double currWood, currVal, minVal = INF, woodRemaining, woodNeeded;
		int minCount = INF;
		for(int i = 0; i < n; i++)
			cin >> x[i] >> y[i] >> val[i] >> len[i];
		for(int taken = 1; taken < (1 << n) - 1; taken++) {
			currWood = 0; currVal = 0;
			curr.clear(); poly.clear();
			for(int i = 0; i < n; i++) {
				if((taken & (1 << i)) != 0) {
					curr.pb(i + 1);
					currVal += val[i];
					...
					poly.pb(point(x[i], y[i]));
				}
			}
			convexHull(poly);
			woodNeeded = perimeter(poly);
			if(currWood - woodNeeded < - EPS) continue;
			if(currVal - minVal < -EPS || (abs(currVal - minVal) < EPS && poly.size() < minCount)) {
				minVal = currVal;
				ans = curr;
				minCount = poly.size();
				woodRemaining = currWood - woodNeeded;
			}
		}
		printf("Forest %d\n", t);
