...
struct point {
	...
	static vec make_vec(point a, point b) {
...
bool inPolygon(vector<point> poly, point p) {
	...
	for (int i = 0; i < (int) poly.size() - 1; ++i) {
		...
		if (abs(theta) < EPS
				|| (abs(theta - M_PI) < EPS && p.between(poly[i], poly[i + 1])))
			return true;
...
vector<point> cutPolygon(vector<point> poly, point a, point b) {
	...
	for (int i = 0; i < poly.size(); i++) {
		...
		double left2 =
				i + 1 == poly.size() ? 0 : v.cross(toVec(a, poly[i + 1]));
		if (left1 > -EPS)
			p.push_back(poly[i]);
		if (left1 * left2 < -EPS) {
...
void convexHull(vector<point>& pts) {
	...
	for (int i = 1; i < pts.size(); ++i) {
		if (i == (int) pts.size() - 1
				|| point::cw(leftMost, pts[i], rightMost)) {
			while (up.size() >= 2
					&& !point::cw(up[(int) up.size() - 2],
							up[(int) up.size() - 1], pts[i]))
				up.pop_back();
			up.push_back(pts[i]);
		}
		if (i == (int) pts.size() - 1
				|| point::ccw(leftMost, pts[i], rightMost)) {
			while (down.size() >= 2
					&& !point::ccw(down[(int) down.size() - 2],
							down[(int) down.size() - 1], pts[i]))
				down.pop_back();
...
vec rotateVec(vec v, double angle) {
...
int main() {
	...
	while (TC--) {
		cin >> N;
		vec direction(1, 0);
		point curr(0, 0), missingPoint, end;
		while (N--) {
			cin >> op >> val;
			if (val == "?") {
				missingCommand = op;
			...
			num = stoi(val);
			if (op == "fd") {
				curr = curr.translate(direction.scale(num));
			} else if (op == "bk") {
				curr = curr.translate(direction.scale(-num));
			} else if (op == "lt") {
				direction = rotateVec(direction, num);
			} else {
				direction = rotateVec(direction, -num);
			}
			end = curr;
		...
		int ans = 0;
		if(missingCommand == "fd" || missingCommand == "bk") {
			ans = (int)round(curr.dist(point(0, 0)));
		} else if(missingCommand == "lt"){
			double angle = point::angle(point(0, 0), missingPoint, curr) * 180 / M_PI;
			if(point::ccw(point(0, 0), curr, missingPoint))
				angle = 360 - angle;
			ans = (int)round(angle) % 360;
		} else {
			double angle = point::angle(point(0, 0), missingPoint, curr) * 180 / M_PI;
			if(point::cw(point(0, 0), curr, missingPoint))
				angle = 360 - angle;
			ans = (int)round(angle) % 360;
		}
		cout << ans << endl;
