...
double solve(point arr[], int n) {
	set<point, ord> cands;
	double d = INF;
	for(int i = 0, j = 0; i < n; i++) {
		while(j < i && abs(arr[i].x - arr[j].x) > d + EPS) {
			cands.erase(arr[j]);
			j++;
		}
		auto start = cands.lower_bound(point(arr[i].x, arr[i].y - d));
		auto end = cands.lower_bound(point(arr[i].x, arr[i].y + d));
		for(; start != end && start != cands.end(); ++start)
			d = min(d, arr[i].dist(*start));
		cands.insert(arr[i]);
	}
	return d;
}
