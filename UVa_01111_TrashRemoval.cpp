...
double getWidth(double angle) {
	vector<point> rotated(tile.size());
	double minX = INF, maxX = -INF;
	for(int i = 0; i < tile.size(); i++) {
		rotated[i] = tile[i].rotate(angle);
		minX = min(minX, rotated[i].x);
		maxX = max(maxX, rotated[i].x);
	}
...
int main() {
	...
	for(int t = 1;true; t++) {
		...
		convexHull(tile);
		double ans = INF;
		for(double i = 0; i < tile.size(); i++) {
			vec v = toVec(tile[i], tile[i + 1]);
			double angle = atan2(v.y, v.x);
			ans = min(ans, getWidth(-angle));
			ans = min(ans, getWidth(- (M_PI / 2 + angle)));
		}
