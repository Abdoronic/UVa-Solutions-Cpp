...
int main() {
	...
	cin.tie(nullptr);
	int n, w, h, x, y;
	for(int t = 1; cin >> n >> w >> h >> x >> y; t++) {
		point f(x, y);
		point a(0, 0), b(w, 0), c(w, h), d(0, h), o(0, 0);
		vector<point> poly = {a, b, c, d, o}, left, right;
		while(n--) {
			cin >> x >> y;
			point a(x, y);
			cin >> x >> y;
			point b(x, y);
			left = cutPolygon(poly, a, b);
			right = cutPolygon(poly, b, a);
			poly = inPolygon(left, f) ? left : right;
		}
		printf("Case #%d: %.3f\n", t, polygonArea(poly));
	}
	return 0;
